...
function CgiHttpContext(onFinished) {
	...
	*/
    this.CgiNodeInfo = () => {
        const drawObject = (title, object) => {
            ...
            for (const name in object) {
                ...
                if (typeof (value) === 'function') continue;
                    ...
                    for (const subName in value) htmlValue += `<tr><td>${subName}</td><td>${value[subName]}</td></tr>`;
        ...
        const session = { id: self.session.id, path: self.session.path, ipAddress: self.session.ipAddress };
...
function CgiHttpSession(request, response) {
	...
	*/
    this.init = function () {
        ...
        const path = Path.join(CgiNodeConfig.SessionPath, this.id);
        ...
        const session = JSON.parse(FS.readFileSync(Path.join(CgiNodeConfig.SessionPath, this.id)));
	...
	*/
    this.save = () => {
        ...
        const session = { id: self.id, path: self.path, ipAddress: self.ipAddress, cookies: self.cookies, data: self.data };
	...
	*/
    this.create = () => {
        ...
        const date = new Date();
        const idString = request.server.remote_addr + request.server.remote_port + request.server.unique_id + date.value + Math.random();
        ...
        const session = { id: id, path: Path.join(CgiNodeConfig.SessionPath, id), ipAddress: request.server.remote_addr, cookies: {}, data: {} };
	...
	*/
    this.cleanUp = () => {
        ...
        const timeOut = (CgiNodeConfig.SessionTimeOut * 1000);
...
function CgiHttpRequest() {
	...
	*/
    this.init = function () {
        ...
        self.post.isMultiPart = (this.headers.content_type.toLowerCase().includes('multipart/form-data'));
	...
	*/
    this.readPost = (onFinishedRead, parseData) => {
        ...
        process.stdin.on('data', data => { self.post.data += data; });
...
var CgiParser = {
    ...
    */
    script: function (id, path, content) {
        ...
        // Create the 
        const script = { id: id, path: path, code: '', content: [] };
        var endIndex = 0;
        let startIndex = 0;
...
const onReady = () => { cgiNodeContext.include(process.env.PATH_TRANSLATED); };
